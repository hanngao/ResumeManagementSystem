...
class Qrcode {
    ...
    private function analysis(){
        ...
        $this->version = -1;
        for($i=1; $i<= MAX_VER; $i++){
            if(Qrcode::getCapacity($i, $this->errorLevel, 
            $this->encode)>=$this->length){
                $this->version = $i;
                $this->dataBits = Qrcode::getCapacity($i, $this->errorLevel, 
                DATA_BITS);
                break;
            }
        }
    ...
    private function errorCorrection(){
        ...
        $data1Len = Qrcode::getECTable($this->version, $this->errorLevel, 
        CW_BLCK1) - $this->ecCw;
        ...
        $data2Len = Qrcode::getECTable($this->version, $this->errorLevel,
        CW_BLCK2)- $this->ecCw;
        if($this->ecBlck2 > 0){ 
